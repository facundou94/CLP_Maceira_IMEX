...
# 4. Cluster Plot with Principal Component Analysis (PCA)
library(factoextra)
pca_res <- prcomp(matint_122_dico_d2d4[, top_actual], scale. = TRUE)
fviz_pca_ind(pca_res, 
             geom.ind = "point", 
             col.ind = as.factor(hkmeans.top30.k2$cluster), 
             palette = "jco", 
             addEllipses = TRUE, 
             ellipse.type = "convex",
...
# 5. Gap Statistic
library(cluster)
gap_stat <- clusGap(matint_122_dico_d2d4[, top_actual], 
                    FUN = hkmeans, K.max = 10, B = 50)
fviz_gap_stat(gap_stat)

# 6. Dunn index
library(clValid)
dunn_index <- dunn(hkmeans.top30.k2$cluster, dist(matint_122_dico_d2d4[, top_actual]))
print(dunn_index)
